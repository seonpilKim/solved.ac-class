...
2 초		256 MB		21437	6122	3334		28.685%
...
풀이
----
단순히 문자 하나하나 비교하는 방식으로는 시간복잡도가 O(NM)이다.
그러나, KMP Algorithm을 이용하면 O(N + M)으로 문자열 검색이 가능하다.

* KMP Algorithm
 - key point : "prefix & suffix", "pi array"
주어진 문자열의 0 ~ i 부분 문자열 중에서 prefix == suffix가 될 수 있는 가장 긴 부분 문자열의 길이를 pi[i]에 저장한다.

예를들어 문자열 "ABAABAB"를 pi배열로 나타내보면,
 - pi[0] = 0, A
 - pi[1] = 0, AB
 - pi[2] = 1, ABA [A]
 - pi[3] = 1, ABAA [A]
 - pi[4] = 2, ABAAB [AB]
 - pi[5] = 3, ABAABA [ABA]
 - pi[6] = 2, ABAABAB [AB] 이다.

그러므로 "ABABABAABAB" 문자열에서 "ABAABAB"를 찾는다면,
(전자의 문자열의 인덱스 = i, 후자의 문자열 인덱스 = j라 가정하자)
 - i = 0, j = 0 : [A = A]
 - i = 1, j = 1 : [B = B]
 - i = 2, j = 2 : [A = A]
 - i = 3, j = 3 : [B ≠ A] -> pi[3-1] = 1이므로 j = 1부터 다시 탐색한다.
 - i = 3, j = 1 : [B = B]
 - i = 4, j = 2 : [A = A]
 - i = 5, j = 3 : [B ≠ A] -> pi[3-1] = 1이므로 j = 1부터 다시 탐색한다.
 - i = 5, j = 1 : [B = B]
 - i = 6, j = 2 : [A = A]
 - i = 7, j = 3 : [A = A]
 - i = 8, j = 4 : [B = B]
 - i = 9, j = 5 : [A = A]
 - i = 10, j = 6 : [B = B] -> Ans : 1, 5

이렇게 KMP Algorithm은 틀렸다는 사실이 아니라, 조금이라도 일치했었다는 정보에 주목하고 미리 전처리 해둔 pi배열을 이용해서 많은 중간 시도를 껑충 건너띌 수 있게 한다.

※참고 사이트 : https://bowbowbow.tistory.com/6